...
blackbox_opt_args = [
  ...
  get_option_specs('init_method', False, 'randomkmeans',
...
class BlackboxOptimiser(object):
  ...
  def _set_up(self):
    ...
    # Copy some parameters from func_caller and options.
    copyable_params_from_func_caller = ['domain_dim', 'domain_bounds']
    for param in copyable_params_from_func_caller:
      setattr(self, param, getattr(self.func_caller, param))
  ...
  def _get_jobs_for_each_worker(self):
    ...
    jobs_each_worker = [len(elem) for elem in self.history.worker_info.values()]
    ...
    return  jobs_each_worker_str + ' (%d)'%(sum(jobs_each_worker))
  ...
  def perform_initial_queries(self):
    ...
    if (hasattr(self.options, 'pre_eval_points') and
        isinstance(self.options.pre_eval_points, np.ndarray)):
      ...
      self.curr_opt_pt = self.pre_eval_points[self.pre_eval_vals.argmax()]
    ...
    if self.options.init_method == 'latin':
      init_points = gpb_utils.latin_hc_sampling(self.domain_dim, num_init_evals)
    elif self.options.init_method == 'random':
      init_points = gpb_utils.random_sampling(self.domain_dim, num_init_evals)
    elif self.options.init_method == 'randomkmeans':
      init_points = gpb_utils.random_sampling_kmeans(self.domain_dim, num_init_evals)
    else:
      raise ValueError('Unknonw init method: %s.'%(self.options.init_method))
...
class RandomOptimiser(BlackboxOptimiser):
  ...
  def __init__(self, func_caller, worker_manager, options=None, reporter=None):
    ...
    super(RandomOptimiser, self).__init__(func_caller, worker_manager, options,
                                          self.reporter)
  ...
  def _determine_next_eval_point(self):
    """ Determine the next point for evaluation. """
    return np.random.random(self.domain_dim)

  def _determine_next_batch_of_eval_points(self):
    """ Determine the next point for evaluation. """
    return np.random.random((self.num_workers, self.domain_dim))
...
def optimise_with_method_on_func_caller(method, func_caller, worker_manager, max_capital,
  ...
  elif method in ['asyTS', 'synTS', 'asyHTS', 'asyUCB', 'asyBUCB', 'synBUCB', 'synUCBPE',
                  'asyEI']:
    from gp_bandit import gpb_from_func_caller
    options.acq = method[3:]
    return gpb_from_func_caller(func_caller, worker_manager, max_capital, options=options,
                                reporter='default')
  else:
